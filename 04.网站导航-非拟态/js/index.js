$(function () {
    ...
    function ShowTip(tip, type) {
        ...
        if ($tip.length == 0) {
            ...
            // 设置样式，也可以定义在css文件中
            $tip = $('' +
                '<span id="tip" style="position:fixed;background-color:rgb(0,220,0,0.5);border-radius:10px;top:75px;left: 50%;z-index:9999;height: 35px;padding: 0 20px;line-height: 35px;">' +
                '</span>');
            $('body').append($tip);
        }
        if(type=='insert'){
            $tip.stop(true).text(tip).css(
                {'margin-left': - $tip.outerWidth() / 2, 'color':'black'}).fadeIn(200).delay(800).fadeOut(800);
        }else if(type=='remove'){
            $tip.stop(true).text(tip).css(
    ...
    $('.mypage .go-to-add').mouseenter(function () {
        $(this).css('backgroundColor','rgba(0,191,255,0.6)')
    })
    ...
    $('.mypage .go-to-add a').click(function () {
        ...
        $('.main-title').find('a').eq(0).addClass('active').siblings('a').removeClass('active')
    })
